...
static void _event_handler(telnet_t *telnet, telnet_event_t *ev, void *user_data)
{
	...
	switch(ev->type)
	{
		...
		case TELNET_EV_SUBNEGOTIATION:
			...
			//	user->width = (((unsigned char)ev->sub.buffer[0])*256) + ev->sub.buffer[1]);
			//	user->height = (((unsigned char)ev->sub.buffer[2])*256) + ev->sub.buffer[3]);
				...
				//user->width = &ev->sub.buffer[0];
				//user->height = &ev->sub.buffer[2];

				printf("NAWS subnegotiation has %d bytes\n", ev->sub.size);
				telnet_printf(telnet, "NAWS subnegotiation has %d bytes\n", ev->sub.size);
			//	telnet_printf(telnet, "ev->sub.buffer is %d %d %d %d\n", ev->sub.buffer[0], ev->sub.buffer[1], ev->sub.buffer[2], ev->sub.buffer[3]);
